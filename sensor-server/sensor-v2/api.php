...
function insert_sensors() {
    //DATABASE variables
    //leggo le variabili $DB_username, $DB_password, etc. dal file di configurazione
    $path = realpath(dirname(__FILE__));
    //chiamata REST a engager 
    $config_filename = $path . "/api_config.php";
    include($config_filename);
    
    $url_engager =  $URL_engager;//"http://192.168.0.19:8080/engager-api/engager"; //TODO mettere nel file di configurazione
    ...
    if ($DB_mysql_active){//se scrivo su mysql, creo i file di log di mysql
        //inizializzo file di log
        $file_log = NULL;
        $file_log = create_file_log();
        //inizializzo il solo file di errore di mysql
        $file_log_error = NULL;
        $file_log_error = create_file_error_log();
        /*if ($DB_phoenix_active)//se uso phoenix, creo i file di log 
            fwrite($file_log_error,  "\nIn base al file di configurazione, INSERISCO ANCHE IN PHOENIX\n". $DB_phoenix_active);*/
    }
    if ($DB_phoenix_active){//se uso phoenix, creo i file di log 
        //inizializzo file di log di phoenix
        $file_log_phoenix = NULL;
        $file_log_phoenix = create_file_log_phoenix();
        //inizializzo il solo file di errore di phoenix
        //scrivo l'errore in due punti, sia nel file generale di log che in quello conntenente i SOLI errori
        $file_log_error_phoenix = NULL;
        $file_log_error_phoenix = create_file_error_log_phoenix();
        /*if ($DB_mysql_active)//se uso phoenix, creo i file di log 
            fwrite($file_log_error_phoenix,  "\nIn base al file di configurazione, INSERISCO ANCHE IN MYSQL\n". $DB_phoenix_active);
        */
    }
    //scrittura
    $now = new DateTime;
    ...
    if ($obj) {//fai tutto, il json e quindi l'array sono ben formati 
        if (is_array($obj)) {
            ...
            if ($DB_phoenix_active) {
                //array inserimento multiplo phoenix
                $array_query_phoenix = array();
                $array_query_user_phoenix = array();
            }
            $last_date = null;
            foreach ($obj as $key => $value) {
                ...
                if (count($value) != 1) {//sensori multipli
                    ...
                    if ($valid_response['error_message'] != "") {
                        ...
                        //log delle chiamate API. Scrivo sia sul file di phoenix che su mysql
                        if($DB_mysql_active){
                            fwrite($file_log, "\n" . $stringData);
                        ...
                        if ($DB_mysql_active)
                             $array_query[$t] = $valid_response['query']; //mysql
                        
                        if ($DB_phoenix_active)
                            $array_query_phoenix[$t] = $valid_response['query_phoenix']; //phoenix
                        ...
                        if ($t == 1) {
                            if ($DB_mysql_active)//mysql
                                $array_query_user[$t] = $valid_response['query2']; //metto direttamente
                            if ($DB_phoenix_active) {
                                $array_query_user_phoenix[$t] = $valid_response['query2_phoenix']; //phoenix
                            }
                            ...
                            if ($last_date != $value['date']) {//se la data è diversa da quella dell'ultimo inserimento
                                ...
                                if ($DB_mysql_active)//mysql
                                    $array_query_user[$t] = $valid_response['query2']; //mysql
                                if ($DB_phoenix_active) {
                                    $array_query_user_phoenix[$t] = $valid_response['query2_phoenix']; //phoenix
                                    //fwrite($file_log_error_phoenix,  "\nPASSO Phoenix MULTI\n" );
                                }
                    ...
                    if ($valid_response['error_message'] != "") {
                        ...
                        //inizio gestione inserimento SINGOLO su mysql
                        if ($DB_mysql_active){
                            ...
                            if (!$conn) {
                                ...
                                if (!$result || !$result2) {
                                    ...
                                    $stringData = $time . " | ERROR (sql error) | " . $ip . ' | Could not enter data (1 sensor): ' . $valid_response['query'] . ' sql error type: ' . mysql_error();
                                    fwrite($file_log, "\n" . $stringData);
                                    //scrivo nel file di errore
                                    fwrite($file_log_error, "\n" . $day . ' | ' . $stringData);
                            ...
                            mysql_close($conn);
                        }//fine gestione inserimento SINGOLO su mysql
                        
                        //...... inizio gestione inserimento SINGOLO in Phoenix
                        if ($DB_phoenix_active){//SE da file di configurazione è attivo phoenix, allora scrivo su phoenix
                            $dns = $DB_host_phoenix;
                            //gestione connessionDatabase Credenziali Phoenix PER UN SOLO SENSORE  
                            foreach ($dns as $key => $value) {
                                //fwrite($file_log_error_phoenix,  "\nEntro: chiave (".$key."), valore(".$value."):\n" );
                                $conn_ODBC = odbc_connect($value,$DB_host_phoenix_user, $DB_host_phoenix_password);
                                if($conn_ODBC || $key == count($DB_host_phoenix)-1 )   //SE sono all'ultimo IP, anche se la connessione NON va, lascio
                                    break;         
                            }
                            if (!$conn_ODBC) {
                                //connessione al DB NON stabilita
                                $stringData = $time . ' | ERROR | ' . $ip . ' | Could not connect: ' ." (Database Credentials: " . $config_filename . ")";
                                fwrite($file_log_phoenix, "\n" . $stringData);
                                //scrivo nel file di errore
                                fwrite($file_log_error_phoenix, "\n" . $day . ' | ' . $stringData);
                                header('Access-Control-Allow-Origin: *');
                                ...
                                $result1_ODBC = odbc_exec($conn_ODBC, $valid_response['query_phoenix']); //inserimento nella tabella sensors di phoenix
                                $result2_ODBC = odbc_exec($conn_ODBC, $valid_response['query2_phoenix']); //inserimento nella tabella user_eval di phoenix
                                $message = '';
                                if (!$result1_ODBC ||  !$result2_ODBC ) {//errore di inserimento su phoenix
                                    $stringData = $time . " | ERROR (phoenix) | " . $ip . ' | Could not enter data (1 sensor): Table sensors: ' . $valid_response['query_phoenix'].
                                                            "Table user_eval ".$valid_response['query2_phoenix'] ;
                                    fwrite($file_log_phoenix, "\n" . $stringData);
                                    //scrivo nel file di errore
                                    fwrite($file_log_error_phoenix, "\n" . $day . ' | ' . $stringData);
                                    //EQUIVALENTE IN PHOENIX di: die('Could not enter data: '.$valid_response['query']. ' sql error'.mysql_error() );
                                } else {//inserimento a buon fine
                                    $stringData = "\n" . $time . ' | ' . $_SERVER['REMOTE_ADDR'] . ' | ' . "1 Sensor stored! ";
                                    fwrite($file_log_phoenix, $stringData);
                                }
                                odbc_close($conn_ODBC);
                            }
                        
                        }//fine gestione connessione phoenix PER UN SOLO SENSORE                        
                    }
            ...
            //variabili usate sia per mysql che per phoenix
            $multiple_insert = false;
            $multiple_insert_user = false;
            
            //inizio creazione query multipla per mysql
            if ($DB_mysql_active){
                //tabella sensor: da array $array_query --> a stringa $multiple_query
                $multiple_query = '';
                foreach ($array_query as $key => $value) {
                    $multiple_insert = true;
                    //fwrite($file_log_test,  "\n".$day.' | '.$key .' | '.$value);
                    //NOTA: devo fare una query sola MULTIPLA per mysql e N UPSERT siìu phoenix
                    if ($key == 1)
                ...
                //tabella user_eval: da array $array_query_user --> a stringa $multiple_query_user
                $multiple_query_user = '';
            ...
            }//fine creazione query multipla per mysql
            
            //inizio creazione query multipla per phoenix
            if ($DB_phoenix_active){//SE da file di configurazione è attivo phoenix, preparo la query su phoenix
                //tabella sensor: da array $array_query_phoenix --> a array $multiple_query_phoenix
                $multiple_query_phoenix = array();
                foreach ($array_query_phoenix as $key => $value) {
                    $multiple_insert = true;
                    if ($key == 1){
                        $pre_all = explode("VALUES", $value);
                        $prefix = $pre_all[0];
                    }
                    $query_part = explode("VALUES", $value);
                    $multiple_query_phoenix[$key] = $prefix ." VALUES ". $query_part[1];
                    //fwrite($file_log_error_phoenix,  "\nARRAY QUERY SENSORI elemento ".$key. ': '.$multiple_query_phoenix[$key]."\n" );
                }
                //tabella user_eval: da array $array_query_user_phoenix--> a array $multiple_query_user_phoenix
                $multiple_query_user_phoenix = array();
                foreach ($array_query_user_phoenix as $key => $value) {
                    $multiple_insert_user = true;
                    if ($key == 1){
                        $pre_all = explode("VALUES", $value);
                        $prefix = $pre_all[0];
                    ...
                    $query_part = explode("VALUES", $value);
                    $multiple_query_user_phoenix[$key] = $prefix ." VALUES ". $query_part[1];                 
                    //fwrite($file_log_error_phoenix,  "\nARRAY QUERY UTENTE elemento ".$key. ': '.$multiple_query_user_phoenix[$key]."\n" );
                }
            }//fine creazione query multipla per phoenix
            //fwrite($file_log_test,  "\n-----".$day.' | '. $ip.' | UID: '. $uid_error);
            if ($multiple_insert) {
                //inizio inserimento multiplo su mysql
                if ($DB_mysql_active){
                    $conn = mysql_connect($DB_host, $DB_username, $DB_password); //prendere da file, connessione mysql
                    if (!$conn) { //gestione query multipla su MYSQL
                        $stringData = $time . ' | ERROR | ' . $ip . ' | Could not connect: ' . mysql_error() .
                                " (Database Credentials: " . $config_filename . ")";
                        fwrite($file_log, "\n" . $stringData);
                        //scrivo nel file di errore
                        ...
                        $message = '';
                        if (!$result || !$result2 ) {//errore di inserimento
                            $stringData = $time . ' | ERROR (mysql) | ' . $ip . ' | Could not enter data (multiple sensors): Table sensors: ' . 
                                        $multiple_query .'Table user_eval: ' . $multiple_query_user . ' sql error ' . mysql_error();
                            fwrite($file_log, "\n" . $stringData);
                            //scrivo nel file di errore
                            fwrite($file_log_error, "\n" . $day . " | " . $stringData);
                            die('Could not enter data: ' . $multiple_query . ' sql error' . mysql_error());
                        } else { //Inserimento andato a buon fine
                    ...
                    mysql_close($conn);
                }//fine inserimento multiplo su mysql
                
                //inizio inserimento multiplo su  phoenix    
                if ($DB_phoenix_active){//gestione connessione phoenix PER SENSORI MULTIPLI   
                    //gestione query multipla su phoenix
                    $dns = $DB_host_phoenix;
                    //gestione connessionDatabase Credentiale phoenix PER UN SOLO SENSORE  
                    foreach ($dns as $key => $value) {
                        fwrite($file_log_error_phoenix,  "\nEntro: chiave (".$key."), valore(".$value."):\n" );
                        $conn_ODBC = odbc_connect($value, $DB_host_phoenix_user, $DB_host_phoenix_password);
                        if($conn_ODBC || ($key == count($DB_host_phoenix)-1) )   
                            break;        
                    }
                    if (!$conn_ODBC) {
                        //connessione al DB NON stabilita
                        ...
                        http_response_code(500);
                        die('Could not connect: ' . mysql_error());
                    } else {//connessione al DB  stabilita
                        //NOTA: la upsert MULTIPLA NON si puo' fare, va fatto un ciclo per ciascun vettore!!
                        $result1_ODBC = '';
                        $result2_ODBC = ''; 
                        foreach ($multiple_query_phoenix as $key => $value) {//inserimento multiplo nella tabella sensors di phoenix
                            $result1_ODBC .= ' | '.odbc_exec($conn_ODBC, $multiple_query_phoenix[$key]);
                        }
                        foreach ($multiple_query_user_phoenix as $key => $value) {//inserimento multiplo nella tabella user_eval di phoenix
                            $result2_ODBC .= ' | '.odbc_exec($conn_ODBC, $multiple_query_user_phoenix[$key]);
                        }
                        $message = '';
                        if (!$result1_ODBC || !$result2_ODBC) {//errore di inserimento
                            foreach ($multiple_query_phoenix as $key => $value) {
                                $stringData = $time . ' | ERROR (phoenix multiple) | ' . $ip . ' | Could not enter data in sensors: ' . $multiple_query_phoenix[$key].' ---';
                            }
                            foreach ( $multiple_query_user_phoenix as $key => $value) {
                                $stringData .= ' | Could not enter data in user_eval: ' . $multiple_query_user_phoenix[$key] ;
                            }
                            fwrite($file_log_phoenix, "\n" . $stringData);
                            //scrivo nel file di errore
                            fwrite($file_log_error_phoenix, "\n" . $day . " | " . $stringData);
                            die('Could not enter data: ' . $multiple_query . ' phoenix error');
                        } else {//Inserimento andato a buon fine
                            $stringData = "\n" . $time . ' | ' . $ip . ' | ' . count($array_query_phoenix) . " sensors stored!";
                            fwrite($file_log_phoenix, $stringData);
                            //$message .= count($array_query). " Sensors stored!"; //check: deve rendere OUTPUT una sola volta o qui osu MYSQL
                            //$message_array['insert_result'] = $message;
                        }
                    }
                }//fine inserimento multiplo su  phoenix    
            }
        } else {
            ...
            $stringData = "\n" . $time . ' | ERROR | ' . $ip . ' | Error in the Json schema format';
            fwrite($file_log, "\n" . $stringData);
            if ($DB_phoenix_active)
                 fwrite($file_log_phoenix, "\n" . $stringData);
            //scrivo nel file di errore
...
function isValidLongitude($longitude) {
    ...
    if (preg_match("/^[-]?((((1[0-7][0-9])|([0-9]?[0-9]))\.(\d+))|180(\.0+)?)$/", $longitude) && is_numeric($longitude)) {
        return true;
    } else {
        return false;
    }
...
//fine gestione uid e device_id
                ...
                if (isset($obj['sensor_name']) && ($obj['sensor_name'] == '') || !isset($obj['sensor_name'])) {
...
//new
    ...
    if ($date_is_valid && $place_is_valid && $type_is_valid && $network_name_is_valid && $sensor_name_is_valid && $mac_address_is_valid && $power_is_valid && $rssi_is_valid && $minor_is_valid &&
            $major_is_valid && $UUID_is_valid && $id_is_valid && $frequency_is_valid && $capabilities_is_valid &&
            $speed_is_valid && $altitude_is_valid && $provider_is_valid && $accuracy_is_valid && $heading_is_valid) {//i dati del json sono tutti scritti in modo corretto 
//inizio gestione parametri uid e device_id
        if ($uid != 'NULL')
            $uid_insert = $uid;
        else
            $uid_insert = $device_id;
//fine gestione parametri uid e device_id

//inizio gestione parametri $cc_x e $cc_y
...
//inizio gestione parametri $cc_x e $cc_y
        ...
        if ($DB_mysql_active){//INIZIO CREAZIONE delle query su MYSQL /*MICHELA*/
            //query tabella sensors (Mysql)
            ...
            $query2 = "INSERT INTO sensors.user_eval (date, device_id, latitude, longitude, cc_x, cc_y, speed, altitude, provider, accuracy, heading, lat_pre_scan, long_pre_scan, date_pre_scan, prev_status, curr_status, appID, version, lang, uid2, profile, lin_acc_x, lin_acc_y, lin_acc_z, avg_lin_acc_magn, avg_speed) VALUES ( '" .
                    ...
                    ")";
            //FINE CREAZIONE delle query su MYSQL
        }/*MICHELA*/
        if ($DB_phoenix_active){//INIZIO CREAZIONE delle query su PHOENIX /*MICHELA*/
            //inizio gestione date          
            $phoenix_date = "to_date('".$obj['date']."')";
            //inizio gestione errori stringhe nelle date per phoenix
            if(($phoenix_date == "to_date('NULL')" ) || ($phoenix_date == "to_date(''0000-00-00 00:00:00'')" ) || ($phoenix_date == "to_date('0000-00-00 00:00:00')"))
                 $phoenix_date ='NULL';    
           $phoenix_date_pre_scan = "to_date(".$obj['date_pre_scan'].")";
            if(($phoenix_date_pre_scan == "to_date('NULL')" ) || ($phoenix_date_pre_scan == "to_date('0000-00-00 00:00:00')") || ($phoenix_date_pre_scan == "to_date(''0000-00-00 00:00:00'')") )
                $phoenix_date_pre_scan ='NULL'; 
            substr_replace($phoenix_date,"to_date(''", "to_date('" );    
            substr_replace($phoenix_date,"'')", "')" );   
            substr_replace($phoenix_date_pre_scan,"to_date(''", "to_date('" );    
            substr_replace($phoenix_date_pre_scan,"'')", "')" );    
            //FINE gestione errori stringhe nelle date per phoenix

            //query tabella sensors (Phoenix)
            $query_phoenix = "UPSERT INTO sensors (idmeasure, date, sender_IP, latitude, longitude, type, network_name, sensor_name, MAC_address, power, rssi, minor, major, UUID, id, frequency, capabilities, speed, altitude, provider, accuracy, heading, frequency_n, power_n, rssi_n, device_model, device_id, date_pre_scan, lat_pre_scan, long_pre_scan, status, prev_status, appID, version, lang, uid2, profile) VALUES (NEXT VALUE FOR sensors_sequence, " .
                    $phoenix_date . ", '" . $ip . "' , " . $latitude_no_spaces . " , " . $longitude_no_spaces . ",'" . $type . "', '" . $obj['network_name'] .
                    "' , '" . $obj['sensor_name'] . "' , '" . $obj['MAC_address'] . "',NULL,NULL, " . $minor_no_space .
                    ", " . $major_no_space . " ,'" . $UUID_no_spaces . "', '" . $id_no_spaces . "',NULL, '" . $obj['capabilities'] .
                    "' , " . $obj['speed'] . " , " . $obj['altitude'] . " , '" . $obj['provider'] . "' , " . $obj['accuracy'] . " , " . $obj['heading'] .
                    " , " . $obj['frequency'] . " , " . $obj['power'] . " , " . $obj['rssi'] . " , " . $device_model . " ," . $uid_insert .
                    " ,  " . $phoenix_date_pre_scan. " , " . $lat_pre_scan_no_spaces . " , " . $long_pre_scan_no_spaces .
                    ",'" . $obj['status'] . "' , '" . $obj['prev_status'] . "', '" . $obj['appID'] . "' , '" . $obj['version'] .
                    "','" . $obj['lang'] . "', '" . $obj['uid2'] . "','" . $obj['profile'] .
                    "')";
            //query tabella user_eval (Phoenix)
            $query2_phoenix = "UPSERT INTO user_eval (user_eval_id, date, device_id, latitude, longitude, cc_x, cc_y, speed, altitude, provider, accuracy, heading, lat_pre_scan, long_pre_scan, date_pre_scan, prev_status, curr_status, appID, version, lang, uid2, profile, lin_acc_x, lin_acc_y, lin_acc_z, avg_lin_acc_magn, avg_speed) VALUES (NEXT VALUE FOR user_eval_sequence,  " .
                   $phoenix_date  . " , " . $uid_insert .
                    " , " . $latitude_no_spaces . " , " . $longitude_no_spaces . " , $cc_x, $cc_y, " . $obj['speed'] . " , " . $obj['altitude'] .
                    " , '" . $obj['provider'] . "' , " . $obj['accuracy'] . " , '" . $obj['heading'] . "' , " . $lat_pre_scan_no_spaces .
                    " , " . $long_pre_scan_no_spaces . " ,  " . $phoenix_date_pre_scan . " , '" . $obj['prev_status'] . "' , '" . $obj['status'] .
                    "', '" . $obj['appID'] . "' , '" . $obj['version'] . "', '" . $obj['lang'] . "', '" . $obj['uid2'] . "','" . $obj['profile'] .
                    "', " . $obj['lin_acc_x'] . " , " . $obj['lin_acc_y'] . ", " . $obj['lin_acc_z'] . ", " . $obj['avg_lin_acc_magn'] . ", " . $obj['avg_speed'] .
                    ")";
        }//FINE CREAZIONE delle query su PHOENIX /*MICHELA*/
    }
    ...
    if($DB_mysql_active){//costruisco la risposta se devo scrivere su mysql /*MICHELA*/
        $query = str_replace(array("\n", "\r"), ' ', $query);
        $query2 = str_replace(array("\n", "\r"), ' ', $query2);
        $response['query'] = $query;
        $response['query2'] = $query2;
    }
    if ($DB_phoenix_active){//costruisco la risposta se devo scrivere su phoenix /*MICHELA*/
        $query_phoenix = str_replace(array("\n", "\r"), ' ', $query_phoenix);
        $query2_phoenix = str_replace(array("\n", "\r"), ' ', $query2_phoenix);
        $response['query_phoenix'] = $query_phoenix;
        $response['query2_phoenix'] = $query2_phoenix;
    } 
    return $response;
}
...
function create_file_log_phoenix() {
    $path = realpath(dirname(__FILE__));
    $log_dir = $path . '/log_api_insert_sensors_phoenix';
    if (!file_exists($log_dir)) {
        mkdir($log_dir, 0777, true);
    }
    $now = new DateTime;
    $today = $now->format('d-m-Y');
    $filename = $log_dir . "/log_api_insert_sensors_phoenix_" . $today . ".txt";
    $file_log = NULL;
    if (!file_exists($filename)) {
        $file_log = fopen($filename, 'a') or die("can't open file");
        $header = "Today LOGs (" . $today . "):\ntime | ERROR (optional) | client IP | message \n";
...
function CallAPI($method, $url, $data = false) {
    ...
    //inizializzo file per stampe di test 
    //$file_log_test = create_file_log_test();
...
function create_file_error_log_phoenix() {
    $path = realpath(dirname(__FILE__));
    $log_dir = $path . '/log_api_insert_sensors_phoenix';
    if (!file_exists($log_dir)) {
        mkdir($log_dir, 0777, true);
    }
    $filename = $log_dir . "/log_api_insert_ERROR_sensor_phoenix.txt";
    $file_log_error = NULL;
    if (!file_exists($filename)) {
        $file_log_error = fopen($filename, 'a') or die("can't open file");
        $header = "INSERT ERRORS registered:\nday | time | ERROR | client IP | message \n";
